                    ...
                    <div className="flex flex-60 flex-wrap px-12">
                        {
                            getDays(activity.month, activity.year).map((day, j) => (
                                ...
                                <button id={i} key={j} className={!activity.activityDays.includes(String(day)) ? "p-2 border-2 border-gray-300 w-12 h-12 hover:border-blue-500 mx-1 my-2 rounded-md": "p-2 bg-green-500 text-white w-12 h-12 mx-1 my-2 rounded-md"} value={day} onClick={(e) => props.handleClick(e)}>{day}</button>
